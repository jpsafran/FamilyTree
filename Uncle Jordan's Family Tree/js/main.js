...
function calculateGenderDistribution() {
  return familyMembers.reduce((acc, member) => {
    acc[member.gender || 'unspecified'] = (acc[member.gender || 'unspecified'] || 0) + 1;
    return acc;
  }, {});
}
...
function findOldestMember() {
  return familyMembers
    .filter(m => m.birth)
    .sort((a, b) => new Date(a.birth) - new Date(b.birth))[0];
}

function findYoungestMember() {
  return familyMembers
    .filter(m => m.birth)
    .sort((a, b) => new Date(b.birth) - new Date(a.birth))[0];
}

function findCommonBirthPlaces() {
  return familyMembers
    .filter(m => m.birthPlace)
    .reduce((acc, member) => {
      acc[member.birthPlace] = (acc[member.birthPlace] || 0) + 1;
      return acc;
    }, {});
}
...
function calculateAverageLifespan() {
  ...
  if (!deceased.length) return 0;
  
  const lifespans = deceased.map(m => {
    const birth = new Date(m.birth);
    const death = new Date(m.death);
    return (death - birth) / (1000 * 60 * 60 * 24 * 365.25);
  });
  
  return (lifespans.reduce((a, b) => a + b, 0) / lifespans.length).toFixed(1);
}

function calculateFamilyLongevity() {
  const dates = familyMembers
    .map(m => m.birth)
    .filter(d => d)
    .map(d => new Date(d));
...
function calculateMonthlyBirths() {
  return familyMembers
    .filter(m => m.birth)
    .reduce((acc, member) => {
      const month = new Date(member.birth).getMonth();
